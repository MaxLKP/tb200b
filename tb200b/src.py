...
class TB200B:
    def __init__(self, port):
        ...
        self.parameters = self.get_sensorparameters()
    ...
    def __checksum(self, response) -> int:
        checksum_response = (~sum(response[1:-1]) & 0xff) + 1 # According to communication protocol
        if checksum_response == response[-1]:
            return 1
        else:
            return 0
    ...
    def get_sensorparameters(self) -> dict:
        ...
        checksum = self.__checksum(response)
        if checksum == 1:
            ...
            sensor_info = {"type": -1., "maxrange": -1., "unit": -1., "sign": -1., "decimals": -1.}
        return sensor_info

    def get_combinedread(self) -> dict:
        self.__write_command(self.commands["command_6"])
        response = self.__read_response(13)
        decimals = self.parameters["decimals"]
        checksum = self.__checksum(response)
        if checksum == 1:
            conc_mgm3 = (float(response[2]) * 256 + float(response[3])) / 10**decimals 
            conc_ppm = (float(response[6]) * 256 + float(response[7])) / 10**decimals
            ...
            combined = {"conc_mgm3": -1., "conc_ppm": -1., "temp": -1., "hum": -1.}
        return combined
    ...
    def get_concentration(self) -> dict:
        ...
        if checksum == 1:
            ...
            concentration = {"conc_mgm3": -1., "conc_ppm": -1.}
